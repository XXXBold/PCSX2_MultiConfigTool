                   ...
                   const wxString &defaultPath)
{
  wxFileDialog fileSelector(parent,
                            _(description),
                            defaultPath.BeforeLast(wxFILE_SEP_PATH),
                            "",
                            fileSelectionMask,
                        ...
                        const wxString &destPath)
{
  ...
  {
    if(wxMessageBox("Folder " + strDestPath + " already exists, continue with copy? All files will be overwritten.",
...
bool runCommandAsync(const wxString &cmd,
                     long &pid)
{
  wxProcess *pProc;
  DEBUG_WXPUTS(__PRETTY_FUNCTION__);

  if(!(pProc=wxProcess::Open(cmd,wxEXEC_ASYNC)))
  {
    wxPuts("wxProcess::Open() failed for command \"" + cmd + "\"");
    return(false);
  }
  pid=pProc->GetPid();
  return(true);
...
bool checkPIDActive(long lPid)
{
  DEBUG_WXPUTS(__PRETTY_FUNCTION__);
  if(lPid < 1)
    return(false);

  return(wxProcess::Exists(lPid));
}
